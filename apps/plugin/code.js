// Figma Design Agent - Plugin Sandbox Code
// Handles Figma API commands via postMessage bridge with callId-based request/response
...
// callId-based postMessage bridge: UI sends { callId, command, params }, sandbox replies { callId, result/error }
figma.ui.onmessage = async (msg) => {
  // Handle callId-based commands from the bridge
  if (msg.callId && msg.command) {
    try {
      const result = await handleCommand(msg.command, msg.params || {});
      figma.ui.postMessage({ callId: msg.callId, result });
    } catch (error) {
      figma.ui.postMessage({ callId: msg.callId, error: error.message || "Error executing command" });
    }
    return;
  }
...
// Selection change listener — push to UI for CopilotKit readable context
